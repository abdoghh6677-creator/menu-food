...
<body>
  <div class="center">
    ...
    <div>${new Date(new Date(order.created_at).getTime() + 4 * 60 * 60 * 1000).toLocaleString('ar-EG')}</div>
  </div>
  <div class="line"></div>
...
<body>
  ...
  <div class="order-info">
    <div><strong>الوقت:</strong> ${new Date(new Date(order.created_at).getTime() + 4 * 60 * 60 * 1000).toLocaleString('ar-EG')}</div>
  ...
  <div class="footer">
    <div>تم إنشاء الفاتورة في ${new Date(new Date().getTime() + 4 * 60 * 60 * 1000).toLocaleString('ar-EG')}</div>
  ...
  const downloadOrdersCsv = (ordersToExport: Order[]) => {
    ...
    const rows = ordersToExport.map((order) => {
      ...
      const date = new Date(new Date(order.created_at).getTime() + 4 * 60 * 60 * 1000).toLocaleString("ar-EG");
